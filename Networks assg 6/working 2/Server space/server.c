...
int getUsernameData(char rcd_username[],char curr_username[],char curr_password[]){
	...
	while (fgets(line, sizeof(line), file)) {
		...
		if(strcmp(username,rcd_username)==0){
...
void AuthUser(int connfd,int type,int logged_in[],char curr_username[],char curr_password[]){
	...
	if(type==0){ // user name
		int match= getUsernameData(buff,curr_username,curr_password);
		if(match==0){
			memset(buff,0,sizeof(buff));
			strcpy(buff,"301");
...
void *cs_interaction(void *vargp){
	...
	int logged_in[2]={0,0};
	char curr_username[500],curr_password[500];
	...
	while(1){
		...
		if(strncmp("USERN",buff,5)==0){
			...
			AuthUser(connfd,0,logged_in,curr_username,curr_password);

		}else if(logged_in[0] && strncmp("PASSWD",buff,6)==0){

			AuthUser(connfd,1,logged_in,curr_username,curr_password);

		}else if(logged_in[0] && logged_in[1] && strncmp("GetFile",buff,7)==0){
		
			bzero(buff,MAX);

			printf("\n=> request for file recieved from client\n");
		...
		}else if(logged_in[0] && logged_in[1] && strncmp(buff,"CreateFile",10)==0){
			printf("Trying to create the file \n");
			...
			printf("file creation Success, Filename = %s\n", filename);
			fclose(fp);
